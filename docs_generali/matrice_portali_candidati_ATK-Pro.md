...
| `mlol_digital_humanities_italia` | Italia | MLOL Digital Humanities `https://www.medialibrary.it/solution/digitalhumanities`; Comune di Bologna / ARBOR `https://www.comune.bologna.it/novita/comunicati-stampa/restart-la-cultura-bologna-e-digitale-concluso-il-progetto-di-potenziamento-delle-tecnologie-ambito`; `https://www.comune.bologna.it/novita/comunicati-stampa/e-online-una-versione-completamente-rinnovata-il-sito-web-dellarchiginnasio`; campione `https://arbor.medialibrary.it/item/185016bb-0d54-412f-9554-5eec348b09f0`; sonda `verify_mlol_dh_technical_probe.py`; pista aggiuntiva `https://archiviocanova.medialibrary.it/` | Controllo 2026-07-24/25: famiglia tecnica promettente per nuove biblioteche digitali locali IIIF. MLOL DH dichiara viewer integrato, documenti IIIF ad alta risoluzione, OCR compatibile con IIIF e piu' portali culturali italiani nello stesso ecosistema. Il campione pubblico ARBOR del Comune di Bologna e presentato come piattaforma basata su IIIF per le raccolte digitali dell'Archiginnasio e delle biblioteche di conservazione/specializzate comunali. Verifica live: la pagina pubblica ARBOR espone `/iiif/{id}/manifest`, un link Mirador e un manifest pieno `https://archiginnasio.jarvis.memooria.org/meta/iiif/185016bb-0d54-412f-9554-5eec348b09f0/manifest`; `verify_manifest_url.py` lo ha scaricato con codice 200 e 103 canvas. Prima esecuzione live di `verify_mlol_dh_technical_probe.py`: 4 candidati trovati, con manifest Jarvis, due viewer Mirador e immagine contenuto S3 sul record pubblico no-login. Verifica esplorativa su `bnpz.medialibrary.it`: host vivo ma riconducibile a MLOL standard di prestito digitale, non a un portale MLOL DH/Jarvis con item pubblici IIIF; escluso quindi da questo sottoramo tecnico. Verifica aggiuntiva su `archiviocanova.medialibrary.it`: portale di collezioni digitalizzate pubblico e promettente, ma il probe su `home/index.aspx` e `media/esplora.aspx` vede solo immagini gallery/copertina, senza item pubblici, viewer o manifest IIIF. La sonda dedicata MLOL DH/Jarvis serve quindi ancora a distinguere con prudenza gli host veramente record-level dai portali MLOL affini ma non ancora esposti come campioni tecnici riusabili | B - famiglia tecnica concreta con un campione pubblico valido e una seconda istanza diretta solo parziale; serve ancora un terzo host realmente record-level prima di qualsiasi integrazione |
| `memooria_jarvis_istanze_italiane` | Italia | istanze pubbliche Jarvis/Memooria da verificare per ente; riferimenti tecnici ufficiali `https://www.memooria.org/servizi-openjarvis/` e case study Estense `https://memooria.org/caso-studio/estense-digital-library/`; campione emerso da ARBOR `https://archiginnasio.jarvis.memooria.org/meta/iiif/185016bb-0d54-412f-9554-5eec348b09f0/manifest` | Controllo 2026-07-24: le fonti ufficiali confermano OpenJarvis come famiglia IIIF/Image/Presentation API con WebAPI REST e gateway di sicurezza, e documentano almeno un'installazione italiana dedicata per la Biblioteca Estense Universitaria. Il micro-scouting su ARBOR ha inoltre fatto emergere un'istanza pubblica reale `archiginnasio.jarvis.memooria.org` con manifest IIIF scaricabile e valido (103 canvas) esposto dalla pagina pubblica `arbor.medialibrary.it/item/...`. La famiglia non e' quindi piu' solo teorica; resta pero da verificare se il pattern sia riusabile in modo prudente su altri host italiani e con termini/licenze item-level leggibili | B - famiglia tecnica concreta con almeno un'istanza italiana pubblica verificata; serve una sonda dedicata prima di qualsiasi promozione a supporto |
