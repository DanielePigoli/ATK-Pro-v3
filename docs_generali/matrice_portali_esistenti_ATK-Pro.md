...
| `biblioteca_digitale_siena` | Biblioteca Digitale Siena | Italia / Toscana | IIIF diretto da viewer item-level `vieweriiif` e manifest pubblico BDS | Basso-Medio | JPG web-resolution consentiti per uso personale, studio e ricerca; citazione BDS obbligatoria e uso commerciale escluso | Supportare solo manifest item-level pubblici BDS, con range esplicito, ritmo prudente e verifica delle condizioni del singolo oggetto |
...
| `dl_ficlit` | FICLIT Digital Library | Italia / Emilia-Romagna | IIIF diretto da item Omeka S o manifest FICLIT; download da immagine diretta canvas | Basso-Medio | Condizioni d'uso specifiche per item set/collezione; corretta citazione richiesta | Supportare solo item/manifest FICLIT pubblici, con range esplicito e verifica termini item-level; evitare tile service per HTTP 500 sui ritagli |
| `archiviostorico_unibo` | Archivio Storico Universita di Bologna | Italia / Emilia-Romagna | IIIF diretto da manifest pubblico `archiviostorico.unibo.it` | Basso-Medio | Manifest pubblici consultabili; diritti e riuso da verificare a livello di singolo oggetto | Supportare solo manifest item-level pubblici, evitare collection di annata, usare range esplicito e controllo diritti/licenza del record |
| `phaidra_unipd` | PHAIDRA Universita di Padova | Italia / Veneto | IIIF diretto da pagina oggetto o manifest PHAIDRA ufficiale | Basso-Medio | Manifest e download pubblici presenti su alcuni oggetti; rights notice e licenza variano a livello item | Supportare solo manifest item-level pubblici, con range esplicito e verifica obbligatoria di rights notice/licenza del singolo oggetto |
...
| `doge_unige` | DOGE Digital Library / Universita di Genova | Italia / Liguria | Manifest sintetico da API DSpace-GLAM e derivati pubblici `IIIF-ALTERNATIVE-*` | Medio | Fonti SBA UniGe indicano `CC BY-SA` per gli oggetti UniGe e `CC0` per i metadati; verificare comunque la licenza del singolo item | Supportare solo derivati pubblici `IIIF-ALTERNATIVE-LD` e PDF `IIIF-ALTERNATIVE-DOWNLOAD`, con range esplicito; escludere `ORIGINAL` e bundle `IIIF-PDF-*` segnati `nodownload` |
...
3. Portali con discovery/fallback ma valore alto: `antenati`, `gallica`,
   `memooria`, `brixiana`, `bub_digitale`, `dl_ficlit`, `biblioteca_digitale_trentina`,
