...
DEFAULT_REPORT = ROOT / ".codex_tmp" / "doge_technical_probe.csv"
DEFAULT_USER_AGENT = "ATK-Pro DOGE technical probe (user-run local verification)"
...
HANDLE_RE = re.compile(r"/handle/20\.500\.12732/(?P<id>\d+)\b", re.IGNORECASE)
...
def _is_doge_host(netloc: str) -> bool:
    return netloc.lower() == "doge.unige.net"
...
def _classify_url(url: str) -> tuple[str, str, str] | None:
    ...
    if item_subresource_match:
        ...
        return "api_item", f"doge_item_{subresource}", item_subresource_match.group("uuid")

    item_match = REST_ITEM_RE.search(path)
    if item_match:
        return "api_item", "doge_rest_item", item_match.group("uuid")

    entity_match = ENTITY_RE.search(path)
    if entity_match:
        return "catalog_record", f"doge_{entity_match.group('entity_type')}", entity_match.group("uuid")

    handle_match = HANDLE_RE.search(path)
    if handle_match:
        return "handle", "doge_handle", handle_match.group("id")

    bundle_match = REST_BUNDLE_RE.search(path)
    if bundle_match:
        subresource = bundle_match.group("subresource")
        if subresource:
            return "bundle", f"doge_bundle_{subresource.lower()}", bundle_match.group("uuid")
        return "bundle", "doge_bundle_metadata", bundle_match.group("uuid")
    ...
    if bitstream_match:
        ...
        if subresource and subresource.lower() == "content":
            return "bitstream", "doge_bitstream_content", bitstream_match.group("uuid")
        if subresource:
            return "bitstream", f"doge_bitstream_{subresource.lower()}", bitstream_match.group("uuid")
        return "bitstream", "doge_bitstream_metadata", bitstream_match.group("uuid")

    if "/server/api/core/bitstreamformats/" in path_lower:
        return "format", "doge_bitstream_format", path.rsplit("/", 1)[-1]
...
def _summarize(candidates: list[ProbeCandidate]) -> str:
    if not candidates:
        return "Nessun item DOGE/DSpace-GLAM, bundle, bitstream, thumbnail o handle candidato trovato."
    counts: dict[str, int] = {}
    roles: dict[str, int] = {}
    for candidate in candidates:
...
def _evaluate_readiness(candidates: list[ProbeCandidate]) -> str:
    ...
    has_record = any(role.startswith("doge_") and role not in {"doge_rest_item"} and role.endswith(("publication", "scientificmaterial", "drawings", "path", "fonds", "orgunit", "place", "person")) for role in roles) or "doge_handle" in roles
    has_api = "doge_rest_item" in roles or any(role.startswith("doge_item_") for role in roles)
    has_surrogates = any(role in roles for role in {"doge_item_thumbnail", "doge_bitstream_thumbnail", "public_image"})
    has_rich_bitstream_graph = any(role in roles for role in {"doge_bitstream_content", "doge_bundle_bitstreams"})

    if has_record and has_api and has_surrogates and has_rich_bitstream_graph:
        return "GO/NO-GO: REVIEW (record, API, preview e bundle tecnici emersi; verificare differenza tra consultazione pubblica e download originale)"
    if has_record and has_api:
        return "GO/NO-GO: HOLD (backend pubblico emerso, ma i surrogate o i bundle di consultazione restano incompleti)"
    if has_record:
        return "GO/NO-GO: HOLD (record pubblico emerso, ma non ancora un pattern tecnico sufficiente)"
    return "GO/NO-GO: NO_GO (nessun record o backend tecnico pubblico riusabile emerso)"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Sonda tecnica prudente per DOGE UniGe: cerca record pubblici, "
            "API DSpace-GLAM, handle, bundle, bitstream, thumbnail e preview "
            "senza dedurre automaticamente un download originale pubblico."
        )
    )
    parser.add_argument("--url", help="Pagina pubblica DOGE/DSpace-GLAM da sondare.")
    ...
    try:
        ...
        print(f"ERRORE: impossibile leggere la pagina DOGE: {exc}", file=sys.stderr)
        return 2
